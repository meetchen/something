...
def manager_windows():
    ...
    def select(num):
        if num == 1:
            ...
            update_stu_windows.score_windows(student_list)
        elif num == 2:
            student_id = s_id.get()
    ...
    def set_cell_value1(event):  # 双击进入编辑状态
        ...
        def saveedit():
            ...
            dao.update_teacher(score_tree.item(item, 'values'))
            entryedit.destroy()
            okb.destroy()
            okb1.destroy()
    ...
    def delete_tree2():
        ...
        if flag:
            dao.delete_teacher(item[0], lesson=item[3])
